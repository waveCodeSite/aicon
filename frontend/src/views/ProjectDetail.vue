...
<script setup>
  ...
  const handleManageChapters = () => {
    ...
    const targetRoute = `/projects/${projectId}/chapters`
    router.push(targetRoute)
  }

  const handleStartGeneration = () => {
    if (!['completed', 'parsed'].includes(project.value?.status)) {
  ...
  const pollProjectStatus = async () => {
    try {
      const statusResponse = await projectsStore.fetchProjectStatus(projectId)
      ...
      if (finalStatuses.includes(statusResponse.project.status)) {
        stopStatusPolling()
        // 更新项目数据
        project.value = statusResponse.project
      }
